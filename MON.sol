    ...
    ERC1155Holder
{
    ...
    mapping(uint256 => MONObj) public MONDetails;

    // max dev allocation is 10% of total supply
    uint256 public maxDevAllocation = 200000 * 10**18;
    uint256 public devAllocation = 25000 * 10**18;
    ...
    // how many tokens to burn every time the MON is given an accessory, the remaining goes to the community and devs
    ...
    mapping(uint256 => uint256) public MONScore;
    mapping(uint256 => uint256) public timeMONBorn;

    // items/benefits for the MON could be anything in the future.
    ...
    event MONConsumed(uint256 nftId, address giver, uint256 itemId);
    event MONMinted(address to);
    event MONFatalized(uint256 nftId, address killer);
    ...
    constructor(address _SMONToken)
        public
        ERC721PresetMinterPauserAutoId(
            "MON",
            "MON",
    ...
    // check that MON didn't starve
    function isMONAlive(uint256 _nftId) public view returns (bool) {
    ...
    function getMONScore(uint256 _nftId) public view returns (uint256) {
        return MONScore[_nftId];
    }
    ...
    function getMONInfo(uint256 _nftId)
        public
        view
        returns (
            uint256 _MON,
    ...
    {
        _MON = _nftId;
        _isAlive = this.isMONAlive(_nftId);
        _score = this.getMONScore(_nftId);
        ...
        _timeMONBorn = timeMONBorn[_nftId];
        _owner = this.ownerOf(_nftId);
        _token = MONDetails[_nftId].token;
        _tokenId = MONDetails[_nftId].id;
    ...
    // get the level the MON is on to calculate the token reward
    ...
    function claimMiningRewards(uint256 nftId) external notPaused {
        require(isMONAlive(nftId), "Your MON is dead, you can't mine");
        require(
    ...
    function buyAccesory(uint256 nftId, uint256 itemId) external notPaused {
        ...
        require(
            ownerOf(nftId) == msg.sender ||
                careTaker[nftId][ownerOf(nftId)] == msg.sender,
            "You must own the MON or be a care taker to buy items"
        );
        // require(isMONAlive(nftId), "Your MON is dead");
        uint256 amountToBurn = amount.mul(burnPercentage).div(100);

        if (!isMONAlive(nftId)) {
            MONScore[nftId] = itemPoints[itemId];
            ...
            MONScore[nftId] = MONScore[nftId].add(itemPoints[itemId]);
        }
        ...
        if (devAllocation <= maxDevAllocation) {
            ...
            SMON.transferFrom(msg.sender, address(this), amount);
            // burn 90% of token, 10% stay for dev and community fund
            SMON.burn(amountToBurn);
        } else {
            SMON.burnFrom(msg.sender, amount);
        }
        emit MONConsumed(nftId, msg.sender, itemId);
    }
    ...
    function mint(address player) public override onlyMinter {
        ...
        timeMONBorn[_tokenIds.current()] = block.timestamp;

        MONDetails[_tokenIds.current()] = MONObj(
            address(this),
            _tokenIds.current(),
            721
        );
    ...
    function fatality(uint256 _deadId, uint256 _tokenId) external notPaused {
        require(
            !isMONAlive(_deadId),
            "The MON has to be starved to claim his points"
        );
        MONScore[_tokenId] = MONScore[_tokenId].add(
            (MONScore[_deadId].mul(60).div(100))
        );
        // delete MONDetails[_deadId];
        _burn(_deadId);
        emit MONFatalized(_deadId, msg.sender);
    }
    ...
    function getFatalityReward(uint256 _deadId) public view returns (uint256) {
        if (isMONAlive(_deadId)) {
            return 0;
        } else {
            return (MONScore[_deadId].mul(50).div(100));
        }
    }
    ...
    // aka WRAP: lets give life to your erc721 token and make it fun to mint $SMON!
    function giveLife(
        uint256 index,
        uint256 _id,
        uint256 nftType
        ...
        if (devAllocation <= maxDevAllocation) {
            ...
            SMON.transferFrom(msg.sender, address(this), giveLifePrice);
            // burn 90% of token, 10% stay for dev and community fund
            SMON.burn(amountToBurn);
        } else {
            SMON.burnFrom(msg.sender, giveLifePrice);
        }
    ...
    // unwrap your MON if it is not dead, and get back your original NFT
    function unwrap(uint256 _MONId) external {
        require(isMONAlive(_MONId), "Your MON is dead, you can't unwrap it");
        transferFrom(msg.sender, address(this), _MONId);
        MONObj memory details = MONDetails[_MONId];
        timeUntilStarving[_MONId] = 1;
        MONScore[_MONId] = 0;
        emit Unwrapped(_MONId);
    ...
    // add care taker so in the future if MONs are sent to tokenizing platforms like niftex we can whitelist and the previous owner could still mine and do interesting stuff.
